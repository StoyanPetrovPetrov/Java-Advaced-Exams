...
public class PastryShop01 {
    public static void main(String[] args) {
        ...
        Integer[] liquidsFromScan = Arrays.stream(scanner.nextLine().split("\\s+")).map(Integer ::parseInt).toArray(Integer[]::new);
        Integer[] ingredientsFromScan = Arrays.stream(scanner.nextLine().split("\\s+")).map(Integer ::parseInt).toArray(Integer[]::new);
        Collections.addAll(liquids,liquidsFromScan);
        Arrays.stream(ingredientsFromScan).forEach(ingredients ::push);
